...
// 誤分類→家庭用フィルタで全面遮断の実害あり。2026-08-13 に再発）。削除しないこと。
//
// 2 文目の「認証情報を求めない」旨は、誤分類の再審査で最も効く材料。フィッシング判定の
// 主要な根拠は「資格情報・決済情報の詐取」なので、それに該当しないことをページ自身に
// 明記しておく（審査人向けの証跡であると同時に、利用者への注意喚起にもなる）。
// 対応手順の正本は docs/PHISHING-MISCLASSIFICATION-RUNBOOK.md。
const OPERATOR_FOOTER = `<p class="op">本ページは uniquetrash.inc が運営する、契約企業向けの書類配信サービスです。<br>当サービスがパスワード・クレジットカード番号などの入力を求めることは一切ありません。</p>`
